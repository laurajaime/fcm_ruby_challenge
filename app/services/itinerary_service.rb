...
class ItineraryService
  ...
  def the_last_segment?(segment, next_segment)
  ...
  def print_itinerary(segments)
    segments.each_with_index do |segment, index|
      ...
      puts presenter.formatted_trip_title(next_segment) if start_of_trip?(segment)
      puts presenter.formatted_trip_info

      puts "\n" if end_of_trip?(segment) && !the_last_segment?(segment, next_segment)
    end
  end
  ...
  def extract_segment(line)
    ...
    if raw_segment[1].in?(Segment::TRANSPORT_TYPES)
      transport_segment(raw_segment)
    else
      hotel_segment(raw_segment)
    end
  end

  def transport_segment(raw_segment)
    {
